...
const FlashCard = () => {
    ...
    return (
        <div className={`flex flex-col gap-10 items-center ${darkMode ? "bg-gray-800" : "bg-white "} justify-center w-full h-full `}>
            ...
            <div onClick={handleFlip} className={`flip-card lg:h-72 lg:w-72 w-60 h-60 flex items-center justify-center  rounded-3xl ${darkMode ? "bg-gray-800 text-white " : "bg-white text-black "}`}>
                <div className={`flip-card-inner ${flip ? 'flipped' : ''}`}>
                    <div className="flip-card-front shadow-2xl">
                        <h2 className='lg:text-2xl text-center text-xl'>
                            Word: <br />{currentWord.word}</h2>
                    </div>
                    <div className="flip-card-back shadow-2xl">
                        <h2 className='lg:text-2xl text-center text-xl'>
                            Translation: <br />{currentWord.translation}</h2>
                    </div>
                </div>
            </div>
